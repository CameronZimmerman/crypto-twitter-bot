...
async function sortTweets(rules) {
  ...
  const twitterData = await request.get(twitterBaseUrl + queryString + '&max_results=100').set({ 'authorization': 'Bearer AAAAAAAAAAAAAAAAAAAAAK1%2BMwEAAAAAaSFzOxhPHKMuZj1jQ4eKxWo%2BPAI%3DKg07QM5LYhsD8znImkTacR0m8BH3grLQrNFiU8DA4vF4dagvoA' });
...
async function checkTweetsAndSendEmail(){
  ...
  if(trending) {
    ...
    const mailOptions = {
      ...
      to: `${process.env.PHONE_NUMBER1}@${process.env.CARRIER1},${process.env.PHONE_NUMBER2}@${process.env.CARRIER2},${process.env.PHONE_NUMBER3}@${process.env.CARRIER3}`,
      subject: '',
      text: trending
    };
